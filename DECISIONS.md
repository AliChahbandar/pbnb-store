...
- **`trailingSlash: 'never'` + `build.format: 'file'`.** By default Astro emits a
  prerendered route as `about/index.html`, so `/about` 307-redirects to `/about/` —
  a redirect hop on every content-page click and a mismatch with the extensionless
  canonicals. Flat-file output serves `/about` directly at 200. The canonical builder
  strips the resulting `.html` from `Astro.url.pathname` so canonical and sitemap
  agree exactly (root keeps its slash, everything else drops it).
...
| Lighthouse mobile — homepage | Perf **100**, A11y **100**, BP **100**, SEO **100** · LCP 1.74 s |
| Lighthouse mobile — collection | Perf **100**, A11y **100**, BP **100**, SEO **100** · LCP 1.52 s |
| All 13 routes | 200, no redirect hops |
